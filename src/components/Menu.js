...
const Menu = () => {
    ...
    const fetchMenu = async () => {
        const data = await fetch('https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=28.668395094474295&lng=77.18985289335252&restaurantId='+resId+'catalog_qa=undefined&submitAction=ENTER');

        const json = await data.json();
        setResInfo(json.data);

        if (json.data) {
            const cards = json.data.cards[2]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.itemCards;
            setItemCards(cards);
        }
    }

    
    const { name, cuisines = [], costForTwoMessage } = resInfo?.card?.info ?? {};
